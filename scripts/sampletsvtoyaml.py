...
for l in f:
    ...
    lineno = lineno + 1
    if len(l) != 4:
        raise inputFileError(f"Input samples file does not have four tab-separated columns! Error detected in line {lineno}")
    if not os.path.exists(path):
        warnings.Warning(f"Input samples file points to a file which does not exist. Problematic path: {path} on line {lineno}. If this is on purpose, ignore this warning")
